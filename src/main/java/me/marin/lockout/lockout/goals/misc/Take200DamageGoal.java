...
public class Take200DamageGoal extends Goal implements TextureProvider, CustomTextureRenderer, HasTooltipInfo {
    ...
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(Constants.NAMESPACE, "textures/custom/take_200_damage.png");
    @Override
    ...
    public boolean renderTexture(GuiGraphicsExtractor context, int x, int y, int tick) {
        ...
        context.itemDecorations(Minecraft.getInstance().font, DISPLAY_ITEM_STACK, x, y, "200");
        return true;
    }
    ...
    public List<String> getSpectatorTooltip() {
        ...
        for (LockoutTeam team : LockoutServer.lockout.getTeams()) {
            double damage = LockoutServer.lockout.damageTaken.getOrDefault(team, 0.0);
            tooltip.add(team.getColor() + team.getDisplayName() + ChatFormatting.RESET + ": " + Math.min(400, (int) damage) + "/200");
        }
        tooltip.add(" ");
